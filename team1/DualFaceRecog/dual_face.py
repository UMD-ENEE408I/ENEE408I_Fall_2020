...
def show_camera():
    ...
    left_cap = cv2.VideoCapture(gstreamer_pipeline(flip_method=2, display_width=640, display_height=480, framerate=60), cv2.CAP_GSTREAMER)
    right_cap = cv2.VideoCapture(gstreamer_pipeline(flip_method=2, sensor_id=1, display_width=640, display_height=480, framerate=60), cv2.CAP_GSTREAMER)
    ...
    for name in os.listdir(KNOWN_FACES_DIR):
        for filename in os.listdir(f"{KNOWN_FACES_DIR}/{name}"): #Get name of folder
            image = face_recognition.load_image_file(f"{KNOWN_FACES_DIR}/{name}/{filename}") #Get name of file
            encoding = face_recognition.face_encodings(image)[0]
            known_faces.append(encoding) #Store face encoding
            known_names.append(name) #Store name of folder
